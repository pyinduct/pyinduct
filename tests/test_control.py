...
class CollocatedTestCase(unittest.TestCase):
    ...
    def test_temp_term(self):
        law = pyinduct.simulation.LawEvaluator(
            pyinduct.simulation.approximate_feedback_law(pyinduct.simulation.FeedbackLaw([self.term1])))
        res = law(self.weights, self.weight_label)["output"]
        self.assertAlmostEqual(res, 6)

    def test_spat_term(self):
        law = pyinduct.simulation.LawEvaluator(
            pyinduct.simulation.approximate_feedback_law(pyinduct.simulation.FeedbackLaw([self.term2])))
        res = law(self.weights, self.weight_label)["output"]
        self.assertAlmostEqual(res, 0)

    def test_product_term(self):
        law = pyinduct.simulation.LawEvaluator(
            pyinduct.simulation.approximate_feedback_law(pyinduct.simulation.FeedbackLaw([self.term3])))
        res = law(self.weights, self.weight_label)["output"]
...
class RadDirichletControlApproxTest(unittest.TestCase):
    ...
    def test_it(self):
        ...
        controller = pyinduct.simulation.Feedback(
            pyinduct.simulation.FeedbackLaw([ph.ScalarTerm(x_at_1, 1), ph.ScalarTerm(xt_at_1, -1)]))
...
class RadRobinControlApproxTest(unittest.TestCase):
    ...
    def test_it(self):
        ...
        controller = pyinduct.simulation.Feedback(
            pyinduct.simulation.FeedbackLaw([ph.ScalarTerm(x_at_l, (beta_i - beta_ti - int_kernel_zz(self.l))),
