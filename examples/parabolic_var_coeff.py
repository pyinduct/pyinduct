...
l = 1.; spatial_domain = (0, l); spatial_disc = 30
...
a1_t = -0; a0_t = -6; alpha_t = 3; beta_t = 3
...
init_eig_funcs_t = np.array([ef.SecondOrderRobinEigenfunction(om, param_t, spatial_domain) for om in eig_freq_t])
init_adjoint_eig_funcs_t = np.array([ef.SecondOrderRobinEigenfunction(om, adjoint_param_t, spatial_domain) for om in eig_freq_t])
...
# simulate
t, q = sim.simulate_state_space(ss_weak, cf.input_function, np.zeros((len(fem_funcs))),
...
win1 = vis.PgAnimatedPlot([evald_x], title="animation")
