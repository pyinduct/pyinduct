...
nodes, fem_base = pi.cure_interval(pi.LagrangeFirstOrder,
                                   spat_domain.bounds,
                                   node_count=n_fem)
act_fem_base = pi.Base(fem_base.fractions[-1])
not_act_fem_base = pi.Base(fem_base.fractions[1:-1])
vis_fems_base = pi.Base(fem_base.fractions[1:])
...
pi.deregister_base("act_base")
pi.deregister_base("sim_base")
pi.deregister_base("vis_base")

win1 = pi.PgAnimatedPlot(eval_d,
                         labels=dict(left='x(z,t)', bottom='z'),
                         save_pics=save_pics)
win2 = pi.PgAnimatedPlot(der_eval_d,
                         labels=dict(left='x\'(z,t)', bottom='z'),
                         save_pics=save_pics)

win3 = pi.PgSurfacePlot(eval_d, title="x(z,t)")
win4 = pi.PgSurfacePlot(der_eval_d, title="x'(z,t)")
