...
class PlotTestCase(unittest.TestCase):
    ...
    def test_plots(self):
        self._test_animated_surface_plot()
        self._test_surface_plot()
        self._test_2d_plot()
        self._test_2d_animated_plot()

    def _test_2d_plot(self):
        pt = vis.Pg2DPlot(data=self.test_2d_data)
        if show_plots:
            pi.show(show_mpl=False)

    def _test_2d_animated_plot(self):
        pt = vis.Pg2DPlot(data=self.test_data, animationAxis=0)
        if show_plots:
            pi.show(show_mpl=False)

    def _test_surface_plot(self):
        pt = vis.PgSurfacePlot(data=self.test_data)
        if show_plots:
            pi.show(show_mpl=False)

    def _test_animated_surface_plot(self):
