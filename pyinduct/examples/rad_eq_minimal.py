...
if test_examples:
    ...
    eig_values, eig_funcs = pi.SecondOrderDirichletEigenfunction.solve_evp_hint(param, l, n=n, scale=scale)
    pi.register_base("eig_funcs", eig_funcs)

    # eigenfunctions target system
    eig_freq = pi.SecondOrderDirichletEigenfunction.eigval_tf_eigfreq(param, eig_val=eig_values)
    eig_freq_t = pi.SecondOrderDirichletEigenfunction.eigval_tf_eigfreq(param_t, eig_val=eig_values)
    scale_t = scale * eig_freq / eig_freq_t
    _, eig_funcs_t = pi.SecondOrderDirichletEigenfunction.solve_evp_hint(param_t, l, eig_freq=eig_freq_t, scale=scale_t)
    pi.register_base("eig_funcs_t", eig_funcs_t)

    # init controller
    x_at_1 = pi.FieldVariable("eig_funcs", location=1)
    xt_at_1 = pi.FieldVariable("eig_funcs_t", weight_label="eig_funcs", location=1)
    controller = pi.Controller(pi.WeakFormulation([pi.ScalarTerm(x_at_1, 1), pi.ScalarTerm(xt_at_1, -1)],
