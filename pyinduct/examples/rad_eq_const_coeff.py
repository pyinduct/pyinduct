...
if __name__ == "__main__" or test_examples:
    ...
    def approximate_observer(sys_lbl, obs_lbl, test_lbl, tar_test_lbl):
        int_kernel_00 = beta_t - beta - (a0_t - a0) / 2 / a2 * l
        l0 = alpha_t - alpha + int_kernel_00

        x_sys = pi.FieldVariable(sys_lbl)
        x_obs = pi.FieldVariable(obs_lbl)
        psi_fem = pi.TestFunction(sys_lbl)
        ...
        observer_gain = pi.ObserverFeedback(
            pi.WeakFormulation(
                [pi.ScalarTerm(psi_fem(0), scale=l0),
    ...
    eig_val, eig_funcs = pi.SecondOrderRobinEigenfunction.solve_evp_hint(
        param, l, n=n)
    ...
    _, eig_funcs_r_t = ReversedRobinEigenfunction.solve_evp_hint(
        param_t, l, eig_val=eig_val, scale=scale_r_t)
    ...
    pi.register_base("eig_funcs", eig_funcs.set_sb_source("fem_funcs"))
    pi.register_base("eig_funcs_t", eig_funcs_t)
    pi.register_base("eig_funcs_r", eig_funcs_r)
    pi.register_base("eig_funcs_r_t", eig_funcs_r_t)
    pi.register_base("fem_funcs", fem_funcs)
    pi.register_base("obs_fem_funcs", fem_funcs)
    pi.register_base("obs_tar_fem_funcs", fem_funcs)
    ...
    field_variable_t = pi.FieldVariable("eig_funcs_t", location=l,
    ...
    trajectory = parabolic.RadFeedForward(
        l, T, param_ti, bound_cond_type, actuation_type, scale=scale_factor)
    ...
    obs_rad_pde, obs_base_labels = approximate_observer(
        "fem_funcs", "obs_fem_funcs", "eig_funcs_r", "eig_funcs_r_t")
    ...
    def obs_ic(z): return .0
    def obs_err_ic(z): return obs_ic(z) - sys_ic(z)
    ics = {rad_pde.name: [pi.Function(sys_ic)],
           obs_rad_pde.name: [pi.Function(obs_ic)],
           obs_err_rad_pde.name: [pi.Function(obs_err_ic)]}

    # spatial domains
    spatial_domains = {rad_pde.name: spatial_domain,
                       obs_rad_pde.name: spatial_domain,
                       obs_err_rad_pde.name: spatial_domain}

    # simulation
    weights = dict()
    sys_ed, obs_ed, obs_err_ed = pi.simulate_systems(
        [rad_pde, obs_rad_pde, obs_err_rad_pde], ics, temporal_domain,
    ...
    # evaluate
    real_obs_err_ed = pi.EvalData(sys_ed.input_data,
                                  obs_ed.output_data - sys_ed.output_data)

    # project back
    info = get_transformation_info("obs_fem_funcs", "eig_funcs_t", 0, 0)
    transformation = get_weight_transformation(info)
    modal_error_weights = np.apply_along_axis(
        transformation, 1, weights["obs_tar_fem_funcs"])
    org_obs_err_ed = get_sim_result(
        "eig_funcs", modal_error_weights, temporal_domain, spatial_domain,
        0, 0, name="observer_error_in_target_coordinates")[0]
    ...
    plots.append(pi.PgAnimatedPlot(
        [real_obs_err_ed, org_obs_err_ed], title="animation", replay_gain=.05))
    ...
    pi.tear_down(("eig_funcs", "eig_funcs_t", "fem_funcs") + \
